...
public class RealAiTitleClientTest {

	@InjectMocks
	private RealAiTitleClient realAiTitleClient;

	@Mock
	private AiTitleFeign aiTitleFeign;
	...
	void shouldReturnGeneratedTitleWhenApiCallSucceeds() {
		...
		given(aiTitleFeign.generateTitle(any(TitleGenerateReq.class)))
			.willReturn(List.of(resDto));

		// when
		String result = realAiTitleClient.generateSummary(firstChat);

		// then
		assertThat(result).isEqualTo(generatedTitle);
		verify(aiTitleFeign).generateTitle(any(TitleGenerateReq.class));
	}
