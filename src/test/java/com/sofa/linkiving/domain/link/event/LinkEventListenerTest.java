...
class LinkEventListenerTest {
	...
	@BeforeEach
	void setUp() {
		reset(summaryQueue, eventPublisher);
	}
	...
	@Test
	@DisplayName("3회 재시도 모두 실패 시 Recover 로직을 타며 FAILED 상태 이벤트를 발행함")
	void shouldPublishFailedEvent_WhenAllRetriesFail() {
		...
		// then
		// PENDING은 발행되지 않아야 하며(예외 발생으로 인해 도달 불가), FAILED가 발행되어야 함
		...
		assertThat(publishedEvent.email()).isEqualTo("fail@test.com");
		assertThat(publishedEvent.response().status()).isEqualTo(SummaryStatus.FAILED);
		assertThat(publishedEvent.response().errorMessage()).isEqualTo("요약 대기열 등록에 실패했습니다.");
	...
	static class RetryTestConfig {
		...
		public LinkEventListener linkEventListener(SummaryQueue summaryQueue,
			...
			return new LinkEventListener(summaryQueue, eventPublisher);
		}
	}
}
