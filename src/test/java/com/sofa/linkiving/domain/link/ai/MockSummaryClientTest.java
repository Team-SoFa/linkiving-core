...
public class MockSummaryClientTest {
	...
	void initialSummary_Detailed() {
		// when
		RagInitialSummaryRes result = client.initialSummary(new RagInitialSummaryReq(1L, 1L, "title", "url", "memo"));
	...
	void comparisonSummary() {
		// when
		RagRegenerateSummaryRes result = client.regenerateSummary(new RagRegenerateSummaryReq(1L, 1L, "url", "old"));
