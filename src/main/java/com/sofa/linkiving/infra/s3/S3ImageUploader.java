...
public class S3ImageUploader implements ImageUploader {
	...
	private String uploadNewImage(String originalUrl) {
		try {
			...
			try (InputStream is = urlConnectionFactory.openStream(originalUrl)) {
				// 스트림으로부터 Content-Type 추론
	...
	private String generateUniqueKeyFromUrl(String url) {
		try {
			...
			return "images/" + UUID.nameUUIDFromBytes(decodedUrl.getBytes()).toString() + "." + extension;
		} catch (Exception e) {
